...
public class ScrollableLinearLayout extends RelativeLayout {
    public static final String TAG = "ScrollableLinearLayout";
     ...
     * 设置最大滚动距离
     *
     * @param maxScrollDistance maxScrollDistance
     */
    ...
    public void computeScroll() {
        ...
        if (mUpFlingScroller.computeScrollOffset()) {
            LogUtil.e(TAG, "mUpFlingScroller mOnScrollListener.canScrollUp() = " + mOnScrollListener.canScrollUp());
            // fling 的
            if (!mOnScrollListener.canScrollUp()) {
                LogUtil.d(TAG, " computeScroll mOnScrollListener.canScrollUp() = " + mOnScrollListener.canScrollUp() + "滑动 ListView");
                // 不能向上滑动了  这时候 只滑动 ListView
                ...
                LogUtil.d(TAG, " computeScroll 滑动自己 currentY = " + currentY);
                scrollTo(0, currentY);
        ...
        if (mHeadScroller.computeScrollOffset()) {
            LogUtil.e(TAG, "mHeadScroller computeScrollOffset");
            // header 滑动
        ...
        if (mDownFlingScroller.computeScrollOffset()) {
            LogUtil.e(TAG, "mDownFlingScroller mOnScrollListener.canScrollUp() = " + mOnScrollListener.canScrollUp());
            // 向下fling
...
//        LogUtil.e(TAG, "dispatchTouchEvent ev = " + ev.getAction());
...
//                        LogUtil.d(TAG, "展开状态 = " + mIsBeingExpand + " , desY = " + desY);
                        // 这里直接调用  滑动
                        scrollTo(0, desY);
                        invalidate();
                        performScrollChanged(desY);
                    } else {
//                        LogUtil.e(TAG, " 不是展开状态  mIsUp = " + mIsUp + " mOnScrollListener.canScrollUp() = " + mOnScrollListener.canScrollUp() + " , mOnScrollListener.canScrollDown() = " + mOnScrollListener.canScrollDown());
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        ListView listView = mOnScrollListener.getListView();
        final int action = ev.getAction();
        LogUtil.e(TAG, "onInterceptTouchEvent ev = " + ev.getAction());
        switch (action & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN:
                listView.getGlobalVisibleRect(mRect);
                LogUtil.d(TAG, "onInterceptTouchEvent mRect = " + mRect + " x = " + ev.getRawX() + " y = " + ev.getRawY());
                if (mRect.contains((int) (ev.getRawX()), (int) (ev.getRawY()))) {
                    isDownInListView = true;
                } else {
                    isDownInListView = false;
                }
                LogUtil.d(TAG, "onInterceptTouchEvent isDownInListView = " + isDownInListView);
                break;
            case MotionEvent.ACTION_MOVE:
                LogUtil.d(TAG, "onInterceptTouchEvent mIsVerticalScroll = " + mIsVerticalScroll + " , isDownInListView = " + isDownInListView + " , mIsBeingExpand = " + mIsBeingExpand);
                if (mIsVerticalScroll && isDownInListView && mIsBeingExpand) {
                    LogUtil.e(TAG, "拦截事件 ----  ");
                    return true;
                }
                break;
        ...
        LogUtil.d(TAG, "onInterceptTouchEvent asdasdasdasdasd");
